...
-- Note CAST is necessary to concatenate a number into a string
SELECT ('Name: ' + park_name + ' Area: ' + CAST(area as VARCHAR)) as park_area
FROM park
ORDER BY park_name;
...
-- Note alternative display beautification with IIF()
SELECT has_camping, AVG(area) AS avg_park_area
--SELECT IIF(has_camping>0, 'Camping', 'No camping') AS camping, AVG(area) AS avg_park_area
FROM park
...
--SELECT park_name, area, iif(area = sl.smallest, 'Smallest', 'Largest') AS size
FROM park p,
	(
		SELECT MIN(area) AS smallest, MAX(area) AS largest
		FROM park
