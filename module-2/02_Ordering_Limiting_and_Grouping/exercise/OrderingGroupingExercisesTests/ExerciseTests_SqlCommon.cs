        ...
        private static void CompareData(DataTable dtExpected, DataTable dtActual)
        {
            for (int i = 0; i < dtExpected.Rows.Count; i++)
            {
                foreach (DataColumn dc in dtExpected.Columns)
                {
                    if (i == 0)
                    {
                        Assert.AreEqual(dtExpected.Rows[i][dc.ColumnName], dtActual.Rows[i][dc.ColumnName], 
                            $"Data returned doesn't match data expected for row {i + 1} in column \"{dc.ColumnName}\". Check that your results are ordered correctly.");
                    }
                    else
                    {
                        Assert.AreEqual(dtExpected.Rows[i][dc.ColumnName], dtActual.Rows[i][dc.ColumnName], 
                            $"Data returned doesn't match data expected for row {i + 1} in column \"{dc.ColumnName}\".");
                    }
                }
            }
        }
